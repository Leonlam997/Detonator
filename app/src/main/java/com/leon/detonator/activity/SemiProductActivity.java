...
public class SemiProductActivity extends BaseActivity {
    ...
    private final Handler myHandler = new Handler(msg -> {
        final int DETECT_FINISH = 2;
        final int DETECT_NEXT_STEP = 3;
        final int DETECT_SEND_COMMAND = 4;
        final int STEP_WRITE_CONFIG = 1;
        final int STEP_WRITE_UID = 2;
        final int STEP_WRITE_PSW = 3;
        final int STEP_WRITE_SHELL = 4;
        final int STEP_CHECK_CONFIG = 5;
        final int STEP_CLEAR_STATUS = 6;
        final int STEP_SCAN = 7;
        final int STEP_READ_SHELL = 8;
        final int STEP_READ_BRIDGE = 9;
        final int STEP_READ_CAPACITY = 10;
        final int STEP_LOCK = 11;
        final int STEP_END = 12;
        final int STEP_FINISHED = 13;
        final int STEP_SCAN_CODE = 14;
        switch (msg.what) {
            case DETECT_FINISH:
                enabledButton(true);
            ...
            case DETECT_SEND_COMMAND:
                ...
                switch (flowStep) {
                    ...
                    case STEP_READ_CAPACITY:
                        ...
                        msg.getTarget().sendEmptyMessageDelayed(DETECT_SEND_COMMAND, ConstantUtils.CHECK_CAPACITY_TIME);
                        return false;
                    case STEP_SCAN_CODE:
            ...
            case DataReceiveListener.HANDLER_RECEIVED_DATA:
                ...
                if (received != null && received.length > 0)
                    if (received[0] == SerialCommand.ALERT_SHORT_CIRCUIT) {
                        myApp.shortCircuit(SemiProductActivity.this, msg.getTarget());
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            ArrayAdapter<String> adapterType = new ArrayAdapter<>(this, R.layout.layout_item_spinner, listType);
            adapterType.setDropDownViewResource(R.layout.layout_item_spinner);
            ...
            btnManual.setOnClickListener(view -> manualInput());
    ...
    private void manualInput() {
        if (btnManual.isEnabled()) {
            final View inputCodeView = LayoutInflater.from(SemiProductActivity.this).inflate(R.layout.layout_dialog_edit, null);
    ...
    private void doTest() {
        if (cbWrite.isChecked() && textViewCode.getText().length() != 13) {
            ...
            myHandler.sendEmptyMessage(DETECT_START);
        }
    }
