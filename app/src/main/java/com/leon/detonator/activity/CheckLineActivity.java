...
public class CheckLineActivity extends BaseActivity {
    ...
    private final Handler myHandler = new Handler(msg -> {
        final int DETECT_SUCCESS = 1;
        final int DETECT_FAIL = 3;
        final int DETECT_NEXT_STEP = 5;
        final int DETECT_ADDRESS = 6;
        final int DETECT_DELAY = 7;
        final int DETECT_SHORT = 8;
        final int DETECT_SEND_COMMAND = 9;
        switch (msg.what) {
...
//                    msg.getTarget().sendEmptyMessageDelayed(DETECT_VOLTAGE, ConstantUtils.BOOT_TIME);
                break;
            ...
            case DETECT_DELAY:
                ...
                break;
            case DETECT_SHORT:
                myApp.shortCircuit(CheckLineActivity.this, msg.getTarget());
                break;
            ...
            case DETECT_SEND_COMMAND:
                switch (flowStep) {
                    ...
                    case STEP_READ_FIELD:
                        ...
                        msg.getTarget().sendEmptyMessageDelayed(DETECT_SEND_COMMAND, ConstantUtils.RESEND_READ_FIELD_CMD_TIMEOUT);
                        break;
                    case STEP_SCAN_CODE:
                        serialPortUtil.sendCmd("", SerialCommand.CODE_SCAN_CODE, ConstantUtils.SCAN_CODE_TIME);
                        msg.getTarget().sendEmptyMessageDelayed(DETECT_FAIL, ConstantUtils.SCAN_CODE_TIME);
                        break;
                }
                break;
            case DataReceiveListener.HANDLER_RECEIVED_DATA:
                ...
                if (received != null && received.length > 0)
                    if (received[0] == SerialCommand.ALERT_SHORT_CIRCUIT) {
                        msg.getTarget().sendEmptyMessage(DETECT_SHORT);
                        ...
                        if (received.length > SerialCommand.CODE_CHAR_AT + 1 && 0 == received[SerialCommand.CODE_CHAR_AT + 1]) {
                            ...
                            switch (flowStep) {
                                case STEP_SCAN:
                                    ...
                                    tvLock.setVisibility((bean.getInside() & SerialCommand.MASK_STATUS_LOCK) == 0 ? View.VISIBLE : View.INVISIBLE);
                                ...
                                case STEP_READ_SHELL:
                                    ...
                                    msg.getTarget().sendEmptyMessage(DETECT_ADDRESS);
                                    break;
                                case STEP_READ_FIELD:
                                    ...
                                    msg.getTarget().sendEmptyMessage(DETECT_DELAY);
                                    flowStep = STEP_END;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            myReceiveListener = DataReceiveListener.getInstance(CheckLineActivity.this, myHandler);
        ...
        if (btnDetect.isEnabled())
            switch (keyCode) {
                case KeyEvent.KEYCODE_1:
                    flowStep = STEP_CLEAR_STATUS;
                    BaseApplication.writeFile(getString(R.string.single_check_detonator));
                    myHandler.sendEmptyMessage(DETECT_CONTINUE);
                    break;
