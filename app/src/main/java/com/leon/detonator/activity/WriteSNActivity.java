...
public class WriteSNActivity extends BaseActivity {
    ...
    private final Handler myHandler = new Handler(msg -> {
        final int HANDLER_SUCCESS = 1;
        final int HANDLER_FAIL = 2;
        final int HANDLER_SCAN = 4;
        ...
        switch (msg.what) {
            ...
            case HANDLER_RESEND:
                ...
                switch (flowStep) {
                    case STEP_WRITE_CLOCK:
                        serialPortUtil.sendCmd(etAddress.getText().toString(), SerialCommand.CODE_WRITE_CLOCK, 0);
                        break;
                    case STEP_WRITE_CONFIG:
                        serialPortUtil.sendCmd(etAddress.getText().toString(), SerialCommand.CODE_SINGLE_WRITE_CONFIG, ConstantUtils.UID_LEN, ConstantUtils.PSW_LEN, ConstantUtils.DETONATOR_VERSION);
                        break;
                    case STEP_WRITE_UID:
                        serialPortUtil.sendCmd(etAddress.getText().toString(), SerialCommand.CODE_WRITE_UID, 0);
                        break;
                    case STEP_WRITE_PSW:
                        serialPortUtil.sendCmd(ConstantUtils.EXPLODE_PSW, SerialCommand.CODE_WRITE_PSW, 0);
                        break;
                    case STEP_WRITE_SHELL:
                        serialPortUtil.sendCmd(etAddress.getText().toString(), SerialCommand.CODE_WRITE_SHELL, 0);
                        break;
                    case STEP_READ_SHELL:
                        serialPortUtil.sendCmd(etAddress.getText().toString(), SerialCommand.CODE_READ_SHELL, ConstantUtils.UID_LEN);
                        break;
                    case STEP_LOCK:
                        serialPortUtil.sendCmd(etAddress.getText().toString(), SerialCommand.CODE_LOCK, 0);
                        break;
                    case STEP_WRITE_DELAY:
                        serialPortUtil.sendCmd(etAddress.getText().toString(), SerialCommand.CODE_SINGLE_WRITE_FIELD, 0, Integer.parseInt(etDelay.getText().toString()), 0);
                        break;
                    case STEP_SCAN_CODE:
                        serialPortUtil.sendCmd("", SerialCommand.CODE_SCAN_CODE, ConstantUtils.SCAN_CODE_TIME);
                        msg.getTarget().sendEmptyMessageDelayed(HANDLER_RESEND, ConstantUtils.RESEND_SCAN_TIMEOUT);
                        return false;
                }
                    ...
                    if (received[0] == SerialCommand.INITIAL_FINISHED) {
                        ...
                        if (startReceive) {
                            ...
                            if (0 == received[SerialCommand.CODE_CHAR_AT + 1]) {
                                switch (flowStep) {
                                    case STEP_READ_SHELL:
                                        if (!etAddress.getText().toString().equals(new String(Arrays.copyOfRange(received, SerialCommand.CODE_CHAR_AT + 2, SerialCommand.CODE_CHAR_AT + 15)))) {
                                    ...
                                    case STEP_WRITE_DELAY:
                                        ...
                                        runOnUiThread(() -> {
                                            try {
                                                etAddress.setText(String.format(Locale.getDefault(), "%s%06d", etAddress.getText().toString().substring(0, 7), Integer.parseInt(etAddress.getText().toString().substring(7)) + 1));
                                            } catch (Exception e) {
                                    ...
                                    case STEP_SCAN_CODE:
                                        if (received.length < 9) {
                                            ...
                                            address = new String(Arrays.copyOfRange(received, SerialCommand.CODE_CHAR_AT + 2, SerialCommand.CODE_CHAR_AT + 15));
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setTitle(R.string.write_detonator_info);
        etAddress = findViewById(R.id.et_address);
        etDelay = findViewById(R.id.et_delay);
        etPeriod = findViewById(R.id.et_interval);
        btnWrite = findViewById(R.id.btn_write);
        btnDelay = findViewById(R.id.btn_delay);
        startReceive = false;
        initData();
        initSound();
        BaseApplication.writeFile(getString(R.string.write_detonator_info));
        try {
        ...
        enabledButton(false);
        etAddress.setFilters(new InputFilter[]{new InputFilter.LengthFilter(13)});
        etAddress.setKeyListener(new NumberKeyListener() {
            @NonNull
            @Override
        ...
        findViewById(R.id.btn_increase).setOnClickListener(v -> etAddress.setText(String.format(Locale.getDefault(), "%s%05d", etAddress.getText().toString().substring(0, 8), (Long.parseLong(etAddress.getText().toString().substring(8)) + 1) % 100000)));
        findViewById(R.id.btn_decrease).setOnClickListener(v -> etAddress.setText(String.format(Locale.getDefault(), "%s%05d", etAddress.getText().toString().substring(0, 8), (Long.parseLong(etAddress.getText().toString().substring(8)) - 1) % 100000)));
        btnWrite.setOnClickListener(v -> {
            if (!etAddress.getText().toString().isEmpty())
                etAddress.setText(etAddress.getText().toString().toUpperCase());
            if (!Pattern.matches(ConstantUtils.SHELL_PATTERN, etAddress.getText().toString()))
                ...
                BaseApplication.writeFile(getString(R.string.write_detonator_info) + ", " + etAddress.getText().toString());
            }
        });
    ...
    private void initData() {
        if (BaseApplication.settings.getSerialNum() != null && !BaseApplication.settings.getSerialNum().isEmpty())
            etAddress.setText(BaseApplication.settings.getSerialNum());
    ...
    public void onDestroy() {
        BaseApplication.settings.setSerialNum(etAddress.getText().toString());
