...
public class DetonatorListAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        viewHolder.address.setTextSize(ConstantUtils.ITEM_TEXT_SIZE);
        ...
        if (BaseApplication.settings.isTunnel()) {
            ...
            viewHolder.row.setTextSize(detonatorBean.getHole() > 99 ? ConstantUtils.ITEM_TEXT_SIZE - 2 : ConstantUtils.ITEM_TEXT_SIZE);
            viewHolder.hole.setText(String.format(Locale.getDefault(), "%d", detonatorBean.getInside()));
            viewHolder.hole.setTextSize(detonatorBean.getInside() > 99 ? ConstantUtils.ITEM_TEXT_SIZE - 2 : ConstantUtils.ITEM_TEXT_SIZE);
        } else {
            viewHolder.row.setText(String.format(Locale.getDefault(), "%d", detonatorBean.getRow()));
            viewHolder.row.setTextSize(detonatorBean.getRow() > 99 ? ConstantUtils.ITEM_TEXT_SIZE - 2 : ConstantUtils.ITEM_TEXT_SIZE);
            viewHolder.hole.setText(String.format(Locale.getDefault(), "%d", detonatorBean.getHole()));
            viewHolder.hole.setTextSize(detonatorBean.getHole() > 99 ? ConstantUtils.ITEM_TEXT_SIZE - 2 : ConstantUtils.ITEM_TEXT_SIZE);
            viewHolder.inside.setText(String.format(Locale.getDefault(), "%d", detonatorBean.getInside()));
            viewHolder.inside.setTextSize(detonatorBean.getInside() > 99 ? ConstantUtils.ITEM_TEXT_SIZE - 2 : ConstantUtils.ITEM_TEXT_SIZE);
        }
