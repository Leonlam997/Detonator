package com.leon.detonator.bean;

public class BaiSeProjectBean {
    private String projectCode;
    private String projectName;
    private String projectType;
    private String burstOrgName;
    private String burstOrgCode;
    private String deviceNO;
    private String burstTime;
    private String idCard;
    private String bursterName;
    private int detonatorCount;
    private boolean isOffline;
    private double offlineTime;
    private String lngLat;
    private String gpsCoordinateSystems;

    public BaiSeProjectBean() {
        projectCode = "";
        projectName = "";
        projectType = "";
        burstOrgName = "";
        burstOrgCode = "";
        deviceNO = "";
        burstTime = "";
        idCard = "";
        bursterName = "";
        lngLat = "";
        gpsCoordinateSystems = "";
    }

    public String getProjectCode() {
        return projectCode;
    }

    public void setProjectCode(String projectCode) {
        this.projectCode = projectCode;
    }

    public String getProjectName() {
        return projectName;
    }

    public void setProjectName(String projectName) {
        this.projectName = projectName;
    }

    public String getProjectType() {
        return projectType;
    }

    public void setProjectType(String projectType) {
        this.projectType = projectType;
    }

    public String getBurstOrgName() {
        return burstOrgName;
    }

    public void setBurstOrgName(String burstOrgName) {
        this.burstOrgName = burstOrgName;
    }

    public String getBurstOrgCode() {
        return burstOrgCode;
    }

    public void setBurstOrgCode(String burstOrgCode) {
        this.burstOrgCode = burstOrgCode;
    }

    public String getDeviceNO() {
        return deviceNO;
    }

    public void setDeviceNO(String deviceNO) {
        this.deviceNO = deviceNO;
    }

    public String getBurstTime() {
        return burstTime;
    }

    public void setBurstTime(String burstTime) {
        this.burstTime = burstTime;
    }

    public String getIdCard() {
        return idCard;
    }

    public void setIdCard(String idCard) {
        this.idCard = idCard;
    }

    public String getBursterName() {
        return bursterName;
    }

    public void setBursterName(String bursterName) {
        this.bursterName = bursterName;
    }

    public int getDetonatorCount() {
        return detonatorCount;
    }

    public void setDetonatorCount(int detonatorCount) {
        this.detonatorCount = detonatorCount;
    }

    public boolean isOffline() {
        return isOffline;
    }

    public void setOffline(boolean offline) {
        isOffline = offline;
    }

    public double getOfflineTime() {
        return offlineTime;
    }

    public void setOfflineTime(double offlineTime) {
        this.offlineTime = offlineTime;
    }

    public String getLngLat() {
        return lngLat;
    }

    public void setLngLat(String lngLat) {
        this.lngLat = lngLat;
    }

    public String getGpsCoordinateSystems() {
        return gpsCoordinateSystems;
    }

    public void setGpsCoordinateSystems(String gpsCoordinateSystems) {
        this.gpsCoordinateSystems = gpsCoordinateSystems;
    }
}
